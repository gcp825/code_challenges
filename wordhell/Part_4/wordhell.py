...
def rank_solver(wordlist,solver_func,args=()):
    ...
    with Pool(processes) as pool:
        collated_results += pool.starmap(solver_process,
                                         zip(wordlist_chunks,repeat(wordlist),repeat(solver_func),repeat(args)))
...
def solver_process(answer_group,wordlist,solver_func,args):
    ...
    for answer in answer_group:
        ...
        for guess in range(6):
            suggestion = solver_func(words,guessed,*args)
...
def alphabetic(words,*args): 
    
    return words[0]


def two_guesses_then_alphabetic(words,guessed,first,second,*args): 
    
    return first if len(guessed) == 0 else second if len(guessed) == 1 else words[0]
